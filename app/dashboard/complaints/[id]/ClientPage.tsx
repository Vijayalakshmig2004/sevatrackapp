...
export default function ComplaintDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const submitFeedback = async () => {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/complaints/${id}/feedback`, {
      method: "POST",
  ...
  const updateWorkflow = async (action: "assign" | "complete" | "close") => {
    setIsUpdating(true)
    setMessage("")
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/complaints/${id}/${action}`, { method: "POST" })
    const data = await response.json()
    if (response.ok) {
      setComplaint(data.complaint)
      setMessage(
